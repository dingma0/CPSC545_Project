...
-   Note: Currently, the paired ATAC and RNA version can be utilized for multiome data. Multiome data can simply be prepared in a way where the algorithm treats RNA and ATAC read-outs from the same cells in Multiome as independent cells. Please skip to [Run Multiome with Paired Version](#multiome) section for guidance.

-   The multiome version, where the CNV calls are weighted based on signal from both ATAC and RNA read-outs is currently under development.
...
### 1. Prepare SNP allele data for both modalities {#prep-snp}
...
### 2. Prepare binned RNA & ATAC inputs {#prep-count}
...
#### Generate combined count matrix {#gen-comb}

Note: This step differs when running Multiome as Paired, please see [here](#multiome).
...
# Running Multiome Data As Paired {#multiome}

There are two ways that the current mixed-modality Numbat extension can indirectly support multiome data: [Multiome as Paired](#multi-pair) or [Multiome as Summed](#multi-sum)

Each modality is still required to have the same feature space, thus, follow the [step 1 fully](#prep-snp) and [step 2](#prep-count) **until** [generating the combined count matrix](#gen-comb) then proceed as follows:

## Multiome as paired {#multi-pair}

One way is for each cell to be treated independently for its ATAC and RNA modality, therefore, the combined count matrix and allele counts will include an \_RNA or \_ATAC suffix for each cell.

#### Generate multiome as paired count matrix

``` r
source(glue("{numbat_repo}/inst/bin/input_prep.R"))
saveRDS(binCnt(
        c(glue("${sample}/{sample}_rna_bin.rds"),
...
#### Generate multiome as paired combined allele dataframes

``` r
source(glue("{numbat_repo}/inst/bin/input_prep.R"))
combine_allele_counts(
        alleleCounts_RNA = glue("${sample}/{sample}_rna_bin.rds"),
        alleleCounts_ATAC = glue("${sample}/{sample}_rna_bin.rds"),
        output_file = glue(${sample}/{sample}_comb_allele_counts.tsv.gz) ,
...
Rscript ${numbat_repo}/inst/bin/run_numbat_multiome.R  \
            ...
            --ref Reference/lambdas_comb_bincnt.rds \
            --gtf  ${binGR}\
            --runMultiomeAsPaired TRUE \
            --parL ${parL}
```

## Multiome as summed {#multi-sum}

The second way, is for each cell to have a unionized feature space for all modalities. This can include summing the counts across ATAC and RNA. PS. We have gotten results through some experimentation, but this has not yet been benchmarked. We recommend your references to also be summed multiome data.

#### Generate multiome as summed count matrix
...
#### Generate combined allele dataframes, such that repeated SNPs from different modalities are harmonized.
